 ...
 * "@":  "&#064;" 
 */
package com.grt.filemanager.model.imp.local.archive.inter.unrar.io;
...
import com.grt.filemanager.model.imp.local.archive.inter.unrar.pack.out.ph.IBlockCipher;
import com.grt.filemanager.model.imp.local.archive.inter.unrar.pack.out.ph.Rijndael;
...
public class ReadOnlyAccessFile extends RandomAccessFile implements
	 ...
	 */
	public ReadOnlyAccessFile(File file, String password)
			throws FileNotFoundException {
		super(file, "r");
			...
			if (sizeToRead > 0) {
				...
				for (int i = 0; i < alignedSize / 16; i++) {
					...
					for (int j = 0; j < out.length; j++) {
						this.data.add((byte) (out[j] ^ AESInit[j % 16])); // 32:114,
																			// 33:101
					}
	...
	public void initAES(Rijndael rin, byte[] salt, byte[] AESInit, byte[] AESKey) {
		...
		try {
			...
			for (int i = 0; i < HashRounds; i++) {
				...
				// pswNum[2] = (byte) (i >> 16);
				// bb.put(pswNum);
				bout.write(new byte[] { (byte) i, (byte) (i >> 8),
						(byte) (i >> 16) });
			...
			for (int i = 0; i < 4; i++)
				for (int j = 0; j < 4; j++)
					AESKey[i * 4 + j] = (byte) (((digest[i * 4] * 0x1000000)
							& 0xff000000
							| ((digest[i * 4 + 1] * 0x10000) & 0xff0000)
							| ((digest[i * 4 + 2] * 0x100) & 0xff00) | digest[i * 4 + 3] & 0xff) >> (j * 8));
			// AESKey[i * 4 + j] = (byte)
			// (((digest[i*4]<<24)&0xff000000+(digest[i*4+1]<<16)&0xff0000+(digest[i*4+2]<<8)&0xff00+digest[i*4+3]&0xff)
			// >> (j * 8));
			// System.out.println(System.currentTimeMillis()-ax);
		} catch (Exception e) {
			e.printStackTrace();
		} catch (OutOfMemoryError e){
			Log.i("chengnan", "outof==0");
			e.printStackTrace();
			System.gc();
			return;
		}
